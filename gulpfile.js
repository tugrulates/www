...
const files = {
  ...
  build: () => gulp.src(files.input).pipe(gulp.dest(files.output)),
...
const site = {
  ...
  command: (command) =>
    spawn("zola", [command].concat(options.drafts ? ["--drafts"] : []), {
      stdio: "inherit",
    }),
